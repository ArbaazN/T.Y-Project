...
<script type="text/javascript">
    ...
    function matchScore(auth,template_1, template_2, succFunction, failFunction) {
        if (template_1 == "" || template_2 == "") {
            alert("Please scan two fingers to verify!!");
            window.location="adlog.php?sta=fail";
            return;
        }
    ...
    function succMatch(result,auth) {
        var idQuality = 100;
        if (result.ErrorCode == 0) {
            if (result.MatchingScore >= idQuality){
                ...
                //alert(auth);
                if(auth=='CRO'){
            window.location="../reg/main.php";}
            if(auth=='BO'){
                window.location="../reg/bomain.php";
            }
        }
            else{
